...
final class PostProcessorRegistrationDelegate {
		...
		if (beanFactory instanceof BeanDefinitionRegistry) {
			...
			for (String ppName : postProcessorNames) {
				...
				if (beanFactory.isTypeMatch(ppName, PriorityOrdered.class)) {
					// BeanDefinitionRegistryPostProcessor 的特殊处理 放入缓存中去
			...
			// 激活postProcessBeanFactory方法，之前激活的是postProcessBeanDefintionRegistry 硬编码设置的BeanDefinitionRegistryPostProcessor
	...
	public static void registerBeanPostProcessors(
		...
		// 容器中找出所有的 BeanPostProcessor bean 定义
		...
		/**
		 * BeanPostProcessorChecker 是一个普通的信息打印，可能会有些情况，当Spring的配置中的后处理还没有被注册就开始了bean的初始化时
		 * 便会打印beanPostProcessorChecker中设置的信息
		 * 成品数+1（新建一个）+ bean定义的总数 =
		 */
		...
		// 第一步，注册所有实现了 priorityOrderedPostProcessors
		sortPostProcessors(priorityOrderedPostProcessors, beanFactory);
